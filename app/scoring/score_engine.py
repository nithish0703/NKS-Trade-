...
# Deterministic layer ordering, each mandatory and worth its configured
# weight. Momentum, candle quality (folded into displacement/structure
# validation), and risk management are intentionally excluded from the
# 120-point confidence score.
_LAYER_WEIGHTS: Mapping[str, float] = {
...
_MANDATORY_LAYERS: tuple[str, ...] = tuple(_LAYER_WEIGHTS.keys())
...
class ConfidenceScoringEngine:
    ...
    def calculate(self, validation_results: Mapping[str, ValidationResult]) -> ConfidenceScoreResult:
        ...
        """
        missing_layers = [layer for layer in _MANDATORY_LAYERS if layer not in validation_results]
        if missing_layers:
            raise ConfidenceScoringError(
        ...
        for layer_name in _MANDATORY_LAYERS:
