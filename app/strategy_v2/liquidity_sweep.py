"""
Check 2: Valid Liquidity Sweep.

Not every wick counts as a valid sweep. This module adds the specific
rules requested for the new strategy on top of the existing confirmed
...
the raw candle series, and adds the wick/body + range-close rules.
"""
...
class ValidSweepResult(BaseModel):
    """Result of validating a confirmed liquidity sweep against the new strategy's stricter rules."""
...
def validate_liquidity_sweep(
    ...
    *,
    ...
    """
    Validate a confirmed liquidity sweep against the new strategy's
    rules: previous high/low broken and reclaimed (guaranteed by
